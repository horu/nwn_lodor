...
if ((GetIsAreaAboveGround(oArea) == AREA_ABOVEGROUND) && (GetIsDay() == TRUE)){
   if (nDark == 1){/* Do Nothing */}
   else {
      SetLocalInt(oPC, "darkness", 1);
      if (GetSubRace(oPC) == "Duergar"){
         DelayCommand(8.0, FloatingTextStringOnCreature("Arrgh! I curse the sun.", oPC, FALSE));
   ...
   if (nDark == 0){/* Do Nothing */}
   else {
      SetLocalInt(oPC, "darkness", 0);
      if (GetSubRace(oPC) == "Duergar"){
         DelayCommand(8.0, FloatingTextStringOnCreature("Ahhh...I welcome the sun's absence.", oPC, FALSE));
            itemproperty ipLoop=GetFirstItemProperty(oSkills);
               while (GetIsItemPropertyValid(ipLoop)){
