...
void main()
{
    ...
    if (GetSubRace(oResting) != "Vampire")
    {
        ...
        else
        {
            ...
            if (((GetIsAreaAboveGround(oArea) == AREA_ABOVEGROUND) && (GetIsDay() == TRUE)) && ((GetSubRace(oResting) ==
                "Duergar") || (GetSubRace(oResting) == "Drow")))
            {
                AssignCommand(oResting, ClearAllActions());
                FloatingTextStringOnCreature(
                    "The daylight makes it too uncomfortable to rest outdoors.",
                    oResting,
                    FALSE);
                return;
            }

            else if ((!(GetIsObjectValid(oItem3))) && (!(GetIsObjectValid(oItem4))) && (!(GetIsObjectValid(oItem5))) &&
                (!(GetIsObjectValid(oItem6))) && (nWater < 1))
            {
                AssignCommand(oPC, ClearAllActions());
                FloatingTextStringOnCreature("You have no drink to rest!", oPC, FALSE);
                return;
            }
            else if (GetIsObjectValid(oItem1)) { DestroyObject(oItem1); }
            ...
            SendMessageToPC(
                oResting,
                "You have " + IntToString(GetLocalInt(oFlask, "water")) + " water in your flask.");
            return;
        }
    }
