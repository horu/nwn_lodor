...
void main()
{
                            ...
                            if (!GetIsFighting(OBJECT_SELF))
                            {
                                if (!GetAssociateState(NW_ASC_MODE_STAND_GROUND))
                                {
                                    if (GetDistanceToObject(GetMaster()) > GetFollowDistance())
                                    {
                                        ...
                                        if (GetAssociateState(NW_ASC_AGGRESSIVE_STEALTH) || GetAssociateState(
                                            NW_ASC_AGGRESSIVE_SEARCH))
                                        {
                                            ...
                                            MyPrintString(
                                                ...
                                                "Assigning Force Follow Command with Search and/or Stealth");
                ...
                else if (!GetAssociateState(NW_ASC_MODE_STAND_GROUND))
                {
                    if (GetIsObjectValid(oMaster))
                    {
                        if (GetCurrentAction(oMaster) != ACTION_REST)
                        {
                            ...
                            {
                                MyPrintString(
                                    "GENERIC SCRIPT DEBUG STRING ********** " + "Assigning Force Follow Normal");
                                ActionForceFollowObject(oMaster, GetFollowDistance());
                            }
                        }
                    }
    ...
    {
        if ((GetMaster(OBJECT_SELF) == OBJECT_INVALID) || (GetSubRace(GetMaster(OBJECT_SELF)) == "Vampire"))
        {
            ExecuteScript("dragon_leave", OBJECT_SELF);
        }
    }
}
