...
object oActivator = GetItemActivator();
    ...
    //This lets the PC fish with a fishing spear
        ...
        FloatingTextStringOnCreature("You need stand in a stream or on a lake shore to use this.", oActivator, FALSE);}}
    ...
    //Orb Of Vanishing
        ...
        if (oUser == oActivator){
            if ((GetTag(oItem) == "faction_report") || (GetTag(oItem) == "ld_sprite_abil") || (GetTag(oItem) == "vampire_bottle") || (GetTag(oItem) == "food_pouch") || (GetTag(oItem) == "lodoc_waterem")){
    ...
    //Holy Symbol Of Uronia
      ...
      if ((GetRacialType(oActivator) == RACIAL_TYPE_GIANT) || (GetRacialType(oActivator) == RACIAL_TYPE_FEY) || (GetRacialType(oActivator) == RACIAL_TYPE_HUMANOID_MONSTROUS)){
        ...
        if (GetLocalInt(oActivator, "quest1b") == 1){
           ...
           FloatingTextStringOnCreature("You threw the holy symbol into the lava lake. It is destroyed.", oPC, FALSE);
    ...
    //Chalice Of Malad (Ogre Quest)
        ...
        if (GetLocalInt(oActivator, "qs1mpc") == 1){
           ...
           FloatingTextStringOnCreature("I need to bring this to the old Malad altar in Drub's Den.", oActivator, TRUE);}}
    ...
    //Recharge Crystal Ball
        ...
        if (GetObjectByTag("LD_DRAGON_BLK") != OBJECT_INVALID){
           ...
           if (GetItemCharges(oChrg) < 1){
               ...
               FloatingTextStringOnCreature("Veeslath took " + IntToString(nGold) + " gold for this service.", oActivator, FALSE);}
           else {
               FloatingTextStringOnCreature("Veeslath wants " + IntToString(nGold) + " gold to recharge this item.", oActivator, FALSE);}
           }
        else {
