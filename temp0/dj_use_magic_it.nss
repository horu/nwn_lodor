...
void main()
{
    ...
    if (GetIsPC(oPC))
    {
        ...
        else if (GetTag(OBJECT_SELF) == "catcacomb_say")
        {
            ...
            {
                SetLocalInt(oPC, "catacomb", 1);
                FloatingTextStringOnCreature(
                    "This place must be long forgotten.  No one will mind if I help myself.",
                    oPC,
                    FALSE);
            }
        }
        ...
        else if (GetTag(OBJECT_SELF) == "boat_sshore1")
        {
            ...
            if (oBoat != OBJECT_INVALID)
            {
                ...
                DelayCommand(
                    2.0,
                    FloatingTextStringOnCreature(
                        "Stay clear of the boat if you are not going out to sea.",
                        oPC,
                        FALSE));
            }
            else
        ...
        else if (GetTag(OBJECT_SELF) == "boat_sshoresay")
        {
            ...
            {
                SetLocalInt(oPC, "seasay476", 1);
                DelayCommand(
                    4.0,
                    FloatingTextStringOnCreature("You can see a small island in the distance.", oPC, FALSE));
            }
        }
