...
void main()
{
    ...
    //Weapon Decays
    if ((GetTag(oItem) == "alter_qst_rimyr2") || (GetTag(oItem) == "alter_qst_belul2"))
    {
        int nRot = GetLocalInt(oItem, "decay");
        if (nRot > 50)
        {
            FloatingTextStringOnCreature("Your weapon was destroyed.", oSpellOrigin, TRUE);
            DestroyObject(oItem);
        }
        else
        {
            int nAdd = nRot + 1;
            SetLocalInt(oItem, "decay", nAdd);
        }
    }
